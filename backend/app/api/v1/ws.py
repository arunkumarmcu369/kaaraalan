...
async def _user_from_token(access_token: Optional[str], expected_role: str) -> Optional[User]:
    if not access_token:
        return None
    payload = safe_decode_token(access_token)
    if not payload or payload.get("type") != "access" or payload.get("role") != expected_role:
        return None
...
async def admin_ws(
    ...
    user = await _user_from_token(access_token, "admin")
    if not user:
...
async def dealer_ws(
    ...
    user = await _user_from_token(access_token, "dealer")
    if not user:
        await websocket.close(code=4403)
        return

    await ws_manager.connect_dealer(user.id, websocket)
    try:
        await websocket.send_json({"type": "connected", "message": "Dealer WebSocket connected"})
        while True:
            data = await websocket.receive_text()
            if data == "ping":
                await websocket.send_json({"type": "pong"})
    except WebSocketDisconnect:
        ws_manager.disconnect_dealer(user.id, websocket)
    except Exception:
        ws_manager.disconnect_dealer(user.id, websocket)
